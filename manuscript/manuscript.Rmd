...
dat <- data.frame(dat)
write.csv(dat, "lcga_dat_scl.csv", row.names = FALSE)
...
step ~~ 0*s", classes = 1:7,
  data = dat)
saveRDS(res_step, paste0("res_step", Sys.Date(), ".RData"))
...
s ~~ 0*q", classes = 1:7,
  data = dat)
saveRDS(res_step_q, paste0("res_step_q", Sys.Date(), ".RData"))
# In case of convergence problems in the first model:
# res_step_q[[4]] <- mxTryHardWideSearch(res_step_q[[4]], extraTries = 100)

} else {
  f <- list.files(pattern = "res_step\\d.+RData$")
  d <- as.Date(gsub("^res_step(.+)\\.RData$", "\\1", f))
  res_step <- readRDS(f[which.max(d)])
  f <- list.files(pattern = "res_step_q\\d.+RData$")
  d <- as.Date(gsub("^res_step_q(.+)\\.RData$", "\\1", f))
  res_step_q <- readRDS(f[which.max(d)])
}
...
# CJ: Consider updating the code below
...
p <- tidySEM::plot_growth(res_step[[2]], rawdata = TRUE, alpha_range = c(0, .05))
...
p_q <- tidySEM::plot_growth(res_step_q[[2]], rawdata = TRUE, alpha_range = c(0, .05))
p_q <- p_q + scale_y_continuous(breaks = seq(0, 1, length.out = 5), labels = labs) + ylab("SCL (rescaled from Box-Cox)")
ggsave("lcga_trajectories_q.svg", p_q, device = "svg", width = 210, height = 120, units = "mm")
```

```{r plotlcga}
knitr::include_graphics("lcga_trajectories.svg")

knitr::include_graphics("lcga_trajectories_q.svg")
```
